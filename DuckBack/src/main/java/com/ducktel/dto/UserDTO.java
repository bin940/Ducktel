...
public class UserDTO {
    ...
    @Size(min = 6, max = 16, message = "아이디는 6자 이상 16자 이하로 입력해주세요.", groups = CreateUser.class)
    @Pattern(regexp = "^[a-zA-Z0-9]+$", message = "아이디는 영문자, 숫자만 입력해주세요.", groups = CreateUser.class)
    private String username;
    @NotBlank(message = "비밀번호를 입력해주세요.", groups = {CreateUser.class, PasswordReset.class})
    @Size(min = 8, max = 16, message = "비밀번호는 8자 이상 16자 이하로 입력해주세요." , groups = {CreateUser.class, PasswordReset.class})
    @JsonProperty(access = JsonProperty.Access.WRITE_ONLY)
    private String password;
    @NotBlank(message = "전화번호를 입력해주세요.", groups = {CreateUser.class, UpdateUser.class})
    @Size(min = 10, max = 11, message = "전화번호는 10자 이상 11자 이하로 입력해주세요.", groups = {CreateUser.class, UpdateUser.class})
    private String phoneNumber;
    @NotBlank(message = "이메일을 입력해주세요.", groups = {CreateUser.class, UpdateUser.class})
    @Email(message = "이메일 형식에 맞게 입력해주세요.", groups = {CreateUser.class, UpdateUser.class})
    private String email;
    @NotBlank(message = "이름을 입력해주세요.", groups = {CreateUser.class, UpdateUser.class})
    @Size(min =2, max =8, message= "이름은 2자 이상 8자 이하로 입력해주세요.", groups = {CreateUser.class, UpdateUser.class})
    private String name;
