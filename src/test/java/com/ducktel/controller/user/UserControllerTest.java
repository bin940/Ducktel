...
class UserControllerTest {
    ...
    void deleteProfile_Success() throws Exception {
        String userId = "user123";
        when(userService.deleteProfile(userId)).thenReturn("회원 탈퇴 성공");

        mockMvc.perform(delete("/api/users/profile/{userId}", userId))
                .andExpect(status().isOk())
    ...
    void getBookingDetail_Success() throws Exception {
        ...
        when(jwtService.getTokenFromHeader("Bearer test-token")).thenReturn("test-token");
        when(jwtService.getUserIdFromToken("test-token")).thenReturn(userId);
        when(bookingService.getBookingDetail(userId)).thenReturn(Collections.singletonList(booking));
    ...
    void deleteBooking_Success() throws Exception {
        ...
        when(jwtService.getTokenFromHeader("Bearer test-token")).thenReturn("test-token");
        when(jwtService.getUserIdFromToken("test-token")).thenReturn(userId);
        when(bookingService.deleteBooking(userId, bookingId)).thenReturn(Collections.singletonList(booking));
    ...
    """;
        ...
        when(jwtService.getTokenFromHeader("Bearer test-token")).thenReturn("test-token");
        when(jwtService.getUserIdFromToken("test-token")).thenReturn(userId);
        when(userService.passWordReset(userId, "newpassword123")).thenReturn("비밀번호 변경 성공");
